...
void sr_send_arp_request(struct sr_instance *sr, struct sr_if *nexthop_if, uint32_t ip_dst)
{
	uint8_t *buf = (uint8_t *) calloc(sizeof(sr_ethernet_hdr_t) + sizeof(sr_arp_hdr_t), 1);
...
void sr_send_arp_reply(struct sr_instance *sr, sr_arp_hdr_t *arp_hdr, struct sr_if *if_node)
{
	uint8_t *buf = (uint8_t *) calloc(sizeof(sr_ethernet_hdr_t) + sizeof(sr_arp_hdr_t), 1);
	sr_ethernet_hdr_t *eth_hdr_reply = calloc(sizeof(sr_ethernet_hdr_t), 1);
	sr_arp_hdr_t *arp_hdr_reply = calloc(sizeof(sr_arp_hdr_t), 1);

	memcpy(eth_hdr_reply->ether_dhost, arp_hdr->ar_sha, ETHER_ADDR_LEN);
	...
	memcpy(buf + sizeof(sr_ethernet_hdr_t), (uint8_t *) arp_hdr_reply, sizeof(sr_arp_hdr_t));

	sr_send_packet(sr, buf, sizeof(sr_ethernet_hdr_t) + sizeof(sr_arp_hdr_t), if_node->name);
	free(eth_hdr_reply);
	free(arp_hdr_reply);
	free(buf);
    ...
    if ((current_time - req->sent) >= 1.0)
	{
		if (req->times_sent >= 5)
		{
			...
			for (pkt = req->packets; pkt != NULL; pkt = pkt->next)
			{
				...
				sr_send_icmp_report(sr, pkt->buf, rt->interface , dst_host_unreachable);
			}
			sr_arpreq_destroy(&sr->cache, req);
            /*return dst_host_unreachable;*/
		}
		else
		{
